...
export default function ProductDetailPage() {
  ...
  const handleColorSelect = (colorValue: string) => {
    ...
    const variant = product.variants.find((v) =>
      v.variantAttributes.some(
        (a: VariantAttribute) => a.name === "Color" && a.value === colorValue
      )
    );
  ...
  const handleSizeSelect = (sizeValue: string) => {
    ...
    const variant = product.variants.find(
      (v) =>
        v.variantAttributes.some(
          (a: VariantAttribute) => a.name === "Size" && a.value === sizeValue
        ) &&
        v.variantAttributes.some(
          (a: VariantAttribute) =>
        ...
        <div className="space-y-8 md:w-lg p-8 bg-secondary border-2 shadow-lg transition-all duration-300">
          ...
          {colors.length > 0 && (
            <div className="space-y-2">
              <p className="text-sm mb-2 font-semibold text-gray-800">Color</p>
          ...
          {sizes.length > 0 && (
            <div className="space-y-2">
              <p className="text-sm mb-2 font-semibold text-gray-800">Size</p>
              <div className="flex gap-2 flex-wrap">
                {sizes.map((size) => {
                  const variant = product.variants.find(
                    (v) =>
                      v.variantAttributes.some(
                        (a) => a.name === "Size" && a.value === size
                      ) &&
                      v.variantAttributes.some(
                        (a) => a.name === "Color" && a.value === currentColor
                      )
                  );
                  const variantStock = variant?.inventory?.stock || 0;
                  const isLowStock =
                    variant?.inventory &&
          ...
          {/* Quantity */}
          <div className="space-y-2">
            <p className="text-sm mb-2 font-semibold text-gray-800">Quantity</p>
