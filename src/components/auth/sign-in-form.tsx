    ...
    if (signInResponse?.error) {
      ...
      switch (signInResponse.error) {
        ...
        default:
          ...
          if (signInResponse.error.toLowerCase().includes("password")) {
            errorMessage =
              "The password you entered is incorrect. Please try again.";
          } else if (signInResponse.error.toLowerCase().includes("email")) {
            errorMessage =
              "No account found with this email address. Please check your email or register.";
          } else {
    ...
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-5">
        ...
        <div className="flex items-center gap-4 my-4">
          ...
          <span className="text-sm text-gray-500">Just Register If You</span>
          <div className="flex-1 h-px bg-gray-300" />
        </div>
